...
export default function EditStudentModal({ isOpen, onClose, onSuccess, student }: EditStudentModalProps) {
  ...
  useEffect(() => {
    if (student && isOpen) {
      ...
      setStatus(student.status || 'Đang học');
      setVoiceType(student.voiceType || '');
      setRank(student.rank || '');
      setLowestNote(student.lowestNote || '');
      setHighestNote(student.highestNote || '');
      setClassName(student.className || '');
      setStartDate(student.startDate || '');
      setBirthYear(student.birthYear || '');
      setDriveLink(student.driveLink || '');
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (voiceType !== undefined) studentData.voiceType = voiceType;
      if (rank !== undefined) studentData.rank = rank;
      if (lowestNote !== undefined) studentData.lowestNote = lowestNote;
      if (highestNote !== undefined) studentData.highestNote = highestNote;
      if (className !== undefined) studentData.className = className;
      if (startDate !== undefined) studentData.startDate = startDate;
      if (birthYear !== undefined) studentData.birthYear = birthYear;
      if (driveLink !== undefined) studentData.driveLink = driveLink;

      await studentService.updateStudent(student._id, studentData);
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error('Error updating student:', err);
      setError(err.message || 'Có lỗi xảy ra khi cập nhật học viên');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-sans">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
          <h3 className="font-bold text-lg text-slate-800">Cập nhật Học Viên</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3A5A42] focus:bg-white transition-all text-slate-900"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3A5A42] focus:bg-white transition-all appearance-none text-slate-900"
                  >
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Link Drive Video (Cố định)</label>
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                    <Activity size={18} />
                  </div>
